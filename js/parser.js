...
(function (global) {
  ...
  function pickBank(text) {
    var cands = [];
    var re = /(?:филиал\s*)?(?:[«"][^»"\n]{2,60}[»"]\s*)?(?:ПАО|ОАО|ЗАО|НАО|ООО|АО)\s*(?:[«"][^»"\n]{2,80}[»"]|[А-ЯЁ][А-Яа-яЁё\-]*[Бб]анк[а-яё]*)/gi;
    var m;
    while ((m = re.exec(text)) !== null) cands.push(m[0]);

    var vtb = text.match(/(Банк\s+[А-ЯЁа-яё\-]{2,30}\s*\((?:ПАО|АО|ООО)\))/i);
    if (vtb) cands.push(vtb[1]);
    var plain = text.match(/(?:^|[^А-Яа-яЁё])(Сбербанк(?:\s+России)?|ВТБ|Тинькофф[^\s,;|]*|Т-Банк)/i);
    if (plain) cands.push(plain[1]);

    var best = '';
    for (var i = 0; i < cands.length; i++) {
      var c = clean(cands[i].replace(/\|/g, ' '));
      if (!BANK_WORD.test(c) || c.length > 140) continue;
